import { useFormStore } from "@/stores/aplikasi/jadwal_shift/form";
...
export function useForm() {
  ...
  const title = ref('Form Data Jadwal Shift');
  const text = ref('Lengkapi informasi detail Jadwal Shift Anda.');
  ...
  watch(() => route.params.id, (newId) => {
    if (newId) {
      ...
      title.value = 'Edit Data Jadwal Shift'; // Ganti judul ketika mode edit
    } else {
      isEditMode.value = false;
      store.CREATE_DATA_ACTION()
      store.resetForm(); // Bersihkan form jika tidak ada ID (mode tambah)
      title.value = 'Tambah Data Jadwal Shift'; // Ganti judul ketika mode tambah
    }
  }, { immediate: true });
