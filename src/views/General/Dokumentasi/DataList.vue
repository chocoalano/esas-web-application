...
const handleBtnChange = (btn) => {
  switch (btn.btn) {
    ...
    case 'refresh':
      ...
      break;
    case 'add':
      router.push({ name: 'general.dokumentasi.create' });
      break;
    case 'export':
      EXPORT_ACTION();
      break;
...
<template>
  <CrudLayout :permission="[
    'view_any_documentations',
    'view_any_documentations',
    'create_documentations',
    'export_documentations',
    'view_documentations',
        ...
        <template v-slot:top>
          <v-toolbar flat class="bg-background">
            <v-toolbar-title class="d-flex align-center">
              <v-icon color="medium-emphasis" icon="mdi-bookshelf" size="x-small" start></v-icon>
        ...
        <template #expanded-row="{ columns, item }">
          <tr>
            <td :colspan="columns.length" class="py-2">
              <v-sheet rounded="lg" border class="pa-10 text-medium-emphasis">
                <div v-html="item.text_docs"></div>
              </v-sheet>
            </td>
          </tr>
        </template>
