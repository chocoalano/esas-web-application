// useForm.js
import { useFormStore } from "@/stores/general/laporan_bug/form"; // Ensure this path is correct
...
export function useForm() {
  ...
  const icon = ref('mdi-bug-check-outline');
  const title = ref('Form Data Laporan Bug');
  const text = ref('Lengkapi informasi detail Laporan Bug Anda.');
  ...
  watch(() => route.params.id, (newId) => {
    if (newId) {
      ...
      title.value = 'Edit Data Laporan Bug';
    } else {
      ...
      title.value = 'Tambah Data Laporan Bug';
    }
  }, { immediate: true });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result?.status === 200 || result?.success) { // Also check for a 'success' property if your API returns it
        ...
        router.push({ name: 'general.laporan-bug.list' }); // Ensure this route name is correct
      } else {
