import { useFormStore } from "@/stores/pengaturan/departemen/form";
...
export function useForm() {
  ...
  const title = ref('Form Data Departemen');
  const text = ref('Lengkapi informasi detail departemen Anda.');
  ...
  watch(() => route.params.id, (newId) => {
    if (newId) {
      ...
      title.value = 'Edit Data Departemen'; // Ganti judul ketika mode edit
    } else {
      isEditMode.value = false;
      store.CREATE_DATA_ACTION()
      store.resetForm(); // Bersihkan form jika tidak ada ID (mode tambah)
      title.value = 'Tambah Data Departemen'; // Ganti judul ketika mode tambah
    }
  }, { immediate: true });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push({ name: 'pengaturan.departemen.list' });
      } else {
