import { useListStore } from "@/stores/pengaturan/departemen/list"
...
export function useList() {
  ...
  const LOAD_ITEMS = async () => {
    try {
      ...
      console.log(error);
      toast.error(`Gagal meload data list departemen: ${error}`)
    } finally {
  ...
  const EXPORT_ACTION = async () => {
    try {
      ...
      link.href = url
      link.download = 'departement.xlsx'
