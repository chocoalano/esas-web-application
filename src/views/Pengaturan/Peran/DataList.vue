...
const handleBtnChange = (btn) => {
  switch (btn.btn) {
    ...
    case 'refresh':
      ...
      break;
    case 'add':
      router.push({ name: 'pengaturan.peran.create' });
      break;
    case 'export':
      EXPORT_ACTION();
      break;
        ...
        <template v-slot:top>
          <v-toolbar flat class="bg-background">
            ...
            <v-btn icon="mdi-delete-sweep" :disabled="pagination.selected.length === 0 || store.isLoading.list"
              ...
              size="small" />
            <v-btn icon="mdi-trash-can-outline" @click="moveTo('pengaturan.peran.tmp')"
          ...
          <tr>
            ...
            <td class="px-2">
              <v-date-input v-model="filters.createdAt" label="Cari dgn tgl dibuat" variant="outlined" density="compact"
                rounded="lg" class="mt-6" clearable prepend-icon="" prepend-inner-icon="mdi-calendar" />
            </td>
            <td class="px-2">
              <v-date-input v-model="filters.updatedAt" label="Cari dgn tgl diperbaharui" variant="outlined"
                density="compact" rounded="lg" class="mt-6" clearable prepend-icon=""
                prepend-inner-icon="mdi-calendar" />
            </td>
            <td class="px-2">
              <v-btn class="flex-grow-1" rounded="lg" variant="tonal" @click="resetFilters"
                :loading="store.isLoading.list" block>
                Reset Filter
              </v-btn>
            </td>
          </tr>
