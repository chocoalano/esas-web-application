import { useFormStore } from "@/stores/pengaturan/peran/form";
...
export function useForm() {
  ...
  const title = ref('Form Data Jenis Izin');
  const text = ref('Lengkapi informasi detail Jenis Izin Anda.');
  ...
  watch(() => route.params.id, (newId) => {
    if (newId) {
      ...
      title.value = 'Edit Data Jenis Izin'; // Ganti judul ketika mode edit
    } else {
      isEditMode.value = false;
      store.CREATE_DATA_ACTION()
      store.resetForm(); // Bersihkan form jika tidak ada ID (mode tambah)
      title.value = 'Tambah Data Jenis Izin'; // Ganti judul ketika mode tambah
    }
  }, { immediate: true });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push({ name: 'pengaturan.peran.list' });
      } else {
  ...
  const addUserAll = () => {
    // Gunakan map untuk membuat array baru berisi semua ID pengguna
    const allUserIds = store.userOptions.map(user => user.id);
    console.log(allUserIds);
    formData.value.user_id = allUserIds;
  };
  const minUserAll = () => {
