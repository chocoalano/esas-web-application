...
const handleBtnChange = (btn) => {
  switch (btn.btn) {
    ...
    case 'refresh':
      ...
      break;
    case 'add':
      router.push({ name: 'pengaturan.posisi.create' });
      break;
    case 'export':
      EXPORT_ACTION();
      break;
...
<template>
  <CrudLayout :permission="[
    'view_any_job_positions',
    'create_job_positions',
    'export_job_positions',
    'view_job_positions', // Tambahkan izin delete jika belum ada untuk CrudLayout
    'view_job_positions', // Tambahkan izin delete jika belum ada untuk CrudLayout
        ...
        <template v-slot:top>
          <v-toolbar flat class="bg-background">
            ...
            <v-btn icon="mdi-delete-sweep" :disabled="pagination.selected.length === 0 || store.isLoading.list"
              ...
              size="small" />
            <v-btn icon="mdi-trash-can-outline" @click="moveTo('pengaturan.posisi.tmp')"
        ...
        <template #expanded-row="{ columns, item }">
          <tr>
            <td :colspan="columns.length" class="py-2">
              <v-sheet rounded="lg" border class="px-2 text-medium-emphasis">
