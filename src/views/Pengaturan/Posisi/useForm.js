import { useFormStore } from "@/stores/pengaturan/posisi/form";
...
export function useForm() {
  ...
  const title = ref('Form Data Posisi/Jabatan');
  const text = ref('Lengkapi informasi detail posisi/jabatan Anda.');
  ...
  watch(() => route.params.id, (newId) => {
    if (newId) {
      ...
      title.value = 'Edit Data Posisi/Jabatan'; // Ganti judul ketika mode edit
    } else {
      isEditMode.value = false;
      store.CREATE_DATA_ACTION()
      store.resetForm(); // Bersihkan form jika tidak ada ID (mode tambah)
      title.value = 'Tambah Data Posisi/Jabatan'; // Ganti judul ketika mode tambah
    }
  }, { immediate: true });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push({ name: 'pengaturan.posisi.list' });
      } else {
