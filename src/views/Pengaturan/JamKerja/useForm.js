import { useFormStore } from "@/stores/pengaturan/jam_kerja/form";
...
export function useForm() {
  ...
  const title = ref('Form Data Jam Kerja');
  const text = ref('Lengkapi informasi detail jam kerja Anda.');
  ...
  watch(() => route.params.id, (newId) => {
    if (newId) {
      ...
      title.value = 'Edit Data Jam Kerja'; // Ganti judul ketika mode edit
    } else {
      isEditMode.value = false;
      store.CREATE_DATA_ACTION()
      store.resetForm(); // Bersihkan form jika tidak ada ID (mode tambah)
      title.value = 'Tambah Data Jam Kerja'; // Ganti judul ketika mode tambah
    }
  }, { immediate: true });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push({ name: 'pengaturan.jam_kerja.list' });
      } else {
