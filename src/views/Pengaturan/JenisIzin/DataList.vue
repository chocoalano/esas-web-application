...
const handleBtnChange = (btn) => {
  switch (btn.btn) {
    ...
    case 'refresh':
      ...
      break;
    case 'add':
      router.push({ name: 'pengaturan.jenis-izin.create' });
      break;
    case 'export':
      EXPORT_ACTION();
      break;
        ...
        <template v-slot:top>
          <v-toolbar flat class="bg-background">
            ...
            <v-btn icon="mdi-delete-sweep" :disabled="pagination.selected.length === 0 || store.isLoading.list"
              ...
              size="small" />
            <v-btn icon="mdi-trash-can-outline" @click="moveTo('pengaturan.jenis-izin.tmp')"
          ...
          <tr>
            <td colspan="6">
              <v-text-field v-model="filters.name" class="ma-2" density="compact" rounded="lg"
                placeholder="Cari nama jenis izin/cuti" hide-details variant="outlined"></v-text-field>
            </td>
            <td colspan="2">
