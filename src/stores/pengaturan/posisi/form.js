...
export const useFormStore = defineStore('posisiForm', () => {
  // --- State ---
  ...
  async function CREATE_DATA_ACTION(id) {
    ...
    try {
      const response = await api.get(`/hris-module/job-positions/create`);
  ...
  async function EDIT_DATA_ACTION(id) {
    isLoading.value = true;
    try {
      const response = await api.get(`/hris-module/job-positions/${id}/edit`);
  ...
  async function CREATE_ACTION(dataToSave) { // Ganti parameter menjadi dataToSave
    ...
    clearErrors();
    try {
      const response = await api.post(`/hris-module/job-positions`, dataToSave);
      return {
        success: true,
  ...
  async function UPDATE_ACTION(id, dataToSave) { // Ganti parameter data menjadi dataToSave
    ...
    clearErrors();
    try {
      const response = await api.put(`/hris-module/job-positions/${id}`, dataToSave);
      return {
        success: true,
