...
export const useListStore = defineStore('posisiList', () => {
  const headers = computed(() => [
    ...
    { title: "Nama Posisi", align: "start", sortable: true, key: "name" },
  ...
  const GET_LIST_PAGINATE = ({ page = 1, itemsPerPage = 10, sortBy, search }) => {
    ...
    return api.get('/hris-module/job-positions', {
  ...
  async function PRINT_ACTION(filter) {
    ...
    try {
      const { data } = await api.get(`/hris-module/job-positions/print`, { params: filter, responseType: 'blob' })
  ...
  async function EXPORT_ACTION(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/hris-module/job-positions/export`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data perusahaan:', error);
      throw error;
  ...
  async function DELETE_ACTION(id) {
    isLoading.value.delete = true;
    try {
      const { status, data } = await api.delete(`/hris-module/job-positions/${id}`)
