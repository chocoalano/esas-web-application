...
export const useListTmpStore = defineStore('peranTmp', () => {
  ...
  const GET_LIST_PAGINATE = ({ page = 1, itemsPerPage = 10, sortBy, search }) => {
    ...
    return api.get('/general-module/roles/deleted', {
  ...
  const POST_RESTORE = (id) => {
    ...
    return api.post(`/general-module/roles/${id}/restore`)
      .then(({ status, data }) => {
  ...
  const FORCE_DELETE = (id) => {
    ...
    return api.delete(`/general-module/roles/${id}/force`)
      .then(({ status, data }) => {
        const success = status === 200
        openSnackbar(success, success
