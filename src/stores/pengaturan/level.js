...
export const useLevelStore = defineStore('level', () => {
  ...
  async function apiListPaginate({ page = 1, itemsPerPage = 10, sortBy, search }) {
    ...
    try {
      const { data } = await api.get('/web-api/level', {
  ...
  async function apiGetShow(id) {
    ...
    try {
      const { data } = await api.get(`/web-api/level/${id}`)
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal mengambil data level:', error);
      throw error;
  ...
  async function apiPostAdd({ company_id, departement_id, name }) {
    isLoading.value.add = true;
    try {
      const { data } = await api.post('/web-api/level', { company_id, departement_id, name })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
  ...
  async function apiPutUpdate({ company_id, departement_id, name }, id) {
    isLoading.value.update = true;
    try {
      const { data } = await api.put(`/web-api/level/${id}`, { company_id, departement_id, name })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
  ...
  async function apiDelete(id) {
    ...
    try {
      const { data } = await api.delete(`/web-api/level/${id}`)
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
  ...
  async function apiDownloadPrint(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/web-api/level/download`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data level:', error);
      throw error;
  ...
  async function apiExport(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/web-api/level/export`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data level:', error);
      throw error;
