...
export const usePerusahaanStore = defineStore('perusahaan', () => {
  ...
  async function apiListPaginate({ page = 1, itemsPerPage = 10, sortBy, search }) {
    ...
    try {
      const { data } = await api.get('/web-api/company', {
  ...
  async function apiGetShow(id) {
    ...
    try {
      const { data } = await api.get(`/web-api/company/${id}`)
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal mengambil data perusahaan:', error);
      throw error;
  ...
  async function apiPostAdd({ companyName, latitude, longitude, radius, address }) {
    isLoading.value.add = true;
    try {
      const { data } = await api.post('/web-api/company', {
        name: companyName,
        latitude,
        longitude,
        radius,
  ...
  async function apiPutUpdate({ companyName, latitude, longitude, radius, address }, id) {
    isLoading.value.update = true;
    try {
      const { data } = await api.put(`/web-api/company/${id}`, {
        name: companyName,
        latitude,
        longitude,
        radius,
  ...
  async function apiDelete(id) {
    ...
    try {
      const { data } = await api.delete(`/web-api/company/${id}`)
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
  ...
  async function apiDownloadPrint(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/web-api/company/download`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data perusahaan:', error);
      throw error;
  ...
  async function apiExport(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/web-api/company/export`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data perusahaan:', error);
      throw error;
