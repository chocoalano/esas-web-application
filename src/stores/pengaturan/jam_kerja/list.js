...
export const useListStore = defineStore('jam_kerjaList', () => {
  ...
  const GET_LIST_PAGINATE = ({ page = 1, itemsPerPage = 10, sortBy, search }) => {
    ...
    return api.get('/hris-module/time-workes', {
  ...
  const GET_FILTER_PAGINATE = (company_id, dept_id, post_id, lvl_id) => {
    isLoading.value.list = true
    return api.get('/hris-module/time-workes/filter-paginate', {
  ...
  async function PRINT_ACTION(filter) {
    ...
    try {
      const { data } = await api.get(`/hris-module/time-workes/print`, { params: filter, responseType: 'blob' })
  ...
  async function EXPORT_ACTION(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/hris-module/time-workes/export`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data perusahaan:', error);
      throw error;
  ...
  async function DELETE_ACTION(id) {
    isLoading.value.delete = true;
    try {
      const { status, data } = await api.delete(`/hris-module/time-workes/${id}`)
