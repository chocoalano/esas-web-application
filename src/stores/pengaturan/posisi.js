...
export const usePosisiStore = defineStore('posisi', () => {
  ...
  async function apiGetShow(id) {
    ...
    try {
      const { data } = await api.get(`/web-api/posisi/${id}`)
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal mengambil data posisi:', error);
      throw error;
  ...
  // Fungsi untuk menambah data posisi dengan form
  async function apiPostAdd({ company_id, departement_id, name }) {
    isLoading.value.add = true;
    try {
      const { data } = await api.post('/web-api/posisi', { company_id, departement_id, name })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menambah data posisi:', error);
      throw error;
    } finally {
  ...
  // Fungsi untuk memperbaharui data posisi dengan form
  async function apiPutUpdate({ company_id, departement_id, name }, id) {
    isLoading.value.update = true;
    try {
      const { data } = await api.put(`/web-api/posisi/${id}`, { company_id, departement_id, name })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal memperbaharui data posisi:', error);
      throw error;
    } finally {
  ...
  async function apiDelete(id) {
    ...
    try {
      const { data } = await api.delete(`/web-api/posisi/${id}`)
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
  ...
  // Fungsi untuk print data posisi
  async function apiDownloadPrint(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/web-api/posisi/download`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data posisi:', error);
      throw error;
  ...
  // Fungsi untuk export data posisi
  async function apiExport(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/web-api/posisi/export`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data posisi:', error);
      throw error;
