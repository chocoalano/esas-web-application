...
export const useListTmpStore = defineStore('levelTmp', () => {
  ...
  const GET_LIST_PAGINATE = ({ page = 1, itemsPerPage = 10, sortBy, search }) => {
    ...
    return api.get('/hris-module/job-levels/deleted', {
  ...
  const POST_RESTORE = (id) => {
    ...
    return api.post(`/hris-module/job-levels/${id}/restore`)
      .then(({ status, data }) => {
  ...
  const FORCE_DELETE = (id) => {
    ...
    return api.delete(`/hris-module/job-levels/${id}/force`)
      .then(({ status, data }) => {
        const success = status === 200
        openSnackbar(success, success
