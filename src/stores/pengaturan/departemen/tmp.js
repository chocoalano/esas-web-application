...
export const useListTmpStore = defineStore('departemenTmp', () => {
  ...
  const GET_LIST_PAGINATE = ({ page = 1, itemsPerPage = 10, sortBy, search }) => {
    ...
    return api.get('/hris-module/departement/deleted', {
  ...
  const POST_RESTORE = (id) => {
    ...
    return api.post(`/hris-module/departement/${id}/restore`)
      .then(({ status, data }) => {
  ...
  const FORCE_DELETE = (id) => {
    ...
    return api.delete(`/hris-module/departement/${id}/force`)
      .then(({ status, data }) => {
        const success = status === 200
        openSnackbar(success, success
