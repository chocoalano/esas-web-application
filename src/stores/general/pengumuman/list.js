...
export const useListStore = defineStore('pengumumanList', () => {
  ...
  const GET_LIST_PAGINATE = ({ page = 1, itemsPerPage = 10, sortBy, search }) => {
    ...
    return api.get('/general-module/announcements', {
  ...
  const GET_FILTER_PAGINATE = (company_id, dept_id, post_id, lvl_id) => {
    isLoading.value.list = true
    return api.get('/general-module/announcements/filter-paginate', {
  ...
  async function PRINT_ACTION(filter) {
    ...
    try {
      const { data } = await api.get(`/general-module/announcements/print`, { params: filter, responseType: 'blob' })
  ...
  async function EXPORT_ACTION(filter) {
    isLoading.value.delete = true;
    try {
      const { data } = await api.get(`/general-module/announcements/export`, { params: filter, responseType: 'blob' })
      return data; // Mengembalikan data agar bisa digunakan
    } catch (error) {
      console.error('Gagal menghapus data perusahaan:', error);
      throw error;
  ...
  async function DELETE_ACTION(id) {
    isLoading.value.delete = true;
    try {
      const { status, data } = await api.delete(`/general-module/announcements/${id}`)
